...
//			queryCount.setParameter("usuario2", usuarioId);
	...
	public String procesarArchivoEmpresa(String fileName, Long empresaId, Long tipoActivacionId, Long loggedUsuarioId) {
		...
		try {
			...
			insertActivacion.setParameter(1, empresa.getId(), LongType.INSTANCE);
			insertActivacion.setParameter(2, estado.getId(), LongType.INSTANCE);
			insertActivacion.setParameter(3, hoy, DateType.INSTANCE);
			
			insertActivacion.setParameter(4, hoy, DateType.INSTANCE);
			insertActivacion.setParameter(5, new Long(1), LongType.INSTANCE);
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (fields.length < 2) {
					...
					if (!ok) {
						...
						insertActivacion.setParameter(8, mid, LongType.INSTANCE);
						insertActivacion.setParameter(9, chip, StringType.INSTANCE);
